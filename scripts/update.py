...
if not RIOT_API_KEY:
    raise RuntimeError("RIOT_API_KEY is missing. Set it in GitHub Actions secrets or your environment.")

PLAYERS = [
    # If either player is NOT NA, set "platform" to the correct platform host prefix (e.g., "euw1", "la2", etc.)
...
# account-v1 and match-v5 are REGIONAL routed. (NA uses AMERICAS)
REGIONAL = "americas"
...
def get_summoner_by_puuid(platform, puuid):
    # summoner-v4 is PLATFORM routed
...
def get_league_entries_by_summoner(platform, encrypted_summoner_id):
    # league-v4 is PLATFORM routed, and expects encryptedSummonerId, not PUUID
    platform = str(platform).strip().lower()
...
def get_match_ids(puuid, start=0, count=20, start_time=None):
    url = f"https://{REGIONAL}.api.riotgames.com/lol/match/v5/matches/by-puuid/{str(puuid).strip()}/ids"
...
def update_player(state, p):
    ...
    st = players.setdefault(label, {
        ...
        # Cache (platform + encryptedSummonerId)
        "platform": p.get("platform", "na1"),
        "summonerId": None,
    ...
    # Ensure new fields exist in older states
    st.setdefault("platform", p.get("platform", "na1"))
    ...
    # 2) Resolve encryptedSummonerId (platform)
    # If this fails with a decrypt error, clear cache and retry once.
    def resolve_summoner_id():
        summ = get_summoner_by_puuid(st["platform"], st["puuid"])
        sid = summ.get("id")  # encryptedSummonerId
        if not sid:
            raise RuntimeError(f"[{label}] summoner-v4 by-puuid returned no 'id': {summ}")
        return sid

    if not st.get("summonerId"):
        st["summonerId"] = resolve_summoner_id()

    # 3) Pull league entries (platform) using encryptedSummonerId
    try:
        entries = get_league_entries_by_summoner(st["platform"], st["summonerId"])
    except requests.HTTPError as e:
        # Some errors are “sticky cache” problems. Clear summonerId and retry once.
        msg = str(e)
        if "decrypting" in msg.lower() or "Bad Request" in msg:
            st["summonerId"] = None
            st["summonerId"] = resolve_summoner_id()
            entries = get_league_entries_by_summoner(st["platform"], st["summonerId"])
        else:
    ...
    for mid in to_process:
        ...
        champs = stats.setdefault("champions", {})
    ...
    # LP gain/loss best-effort attribution (only if exactly one match processed this run)
    if lp_delta is not None and len(new_match_results) == 1:
        _, w = new_match_results[0]
        deltas = stats.setdefault("lpDelta", {"wins": [], "losses": []})
        if w:
            deltas["wins"].append(lp_delta)
        else:
            deltas["losses"].append(abs(lp_delta))

    wins = stats.get("lpDelta", {}).get("wins", []) or []
    losses = stats.get("lpDelta", {}).get("losses", []) or []
...
def main():
    ...
    for p in PLAYERS:
        try:
            ...
            # Do not kill the whole run; log and continue.
            print(f"[{p.get('label','?')}] ERROR: {e}. Skipping this player.")

    now_ny = datetime.now(timezone.utc).astimezone(ZoneInfo("America/New_York"))
    state["updatedAt"] = {"iso": now_ny.isoformat(), "display": now_ny.strftime("Last Updated: %-I:%M%p on %-m/%-d")}
    save_state(state)
